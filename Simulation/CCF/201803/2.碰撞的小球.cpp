...
const int maxn = 100;  // 小球最大数量
int n, l, t;           // 小球数量，线段长度，模拟运动的时长

class Node {
public:
    int id;            // 小球编号
    int pos;           // 位置
    int dir;           // 运动方向，1 表示向右，-1 表示向左
} balls[maxn];
...
int main() {
    ...
    sort(balls, balls + n, cmpPos); // 所有小球按位置排序
    for (int i = 0; i < t; i++) {
        // 时间流逝一秒
        for (int j = 0; j < n; j++) {
            // 针对每一个小球
            balls[j].pos += balls[j].dir;
            if (balls[j].pos == 0 || balls[j].pos == l) {
                // 到达线段两端，则方向调转
                balls[j].dir = -balls[j].dir;
            }
        }

        // 联想蚂蚁感冒，不可能出现三球相撞的情况，而只可能是两球相撞
        // 在排序之后，两球碰撞的判断只需判断相邻的两球
        for (int j = 1; j < n; j++) {
